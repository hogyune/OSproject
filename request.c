...
void requestServeDynamic(rio_t *rio, int fd, char *filename, char *cgiargs, int bodyLength, double arrivalTime)
{
  ...
  //
	char buf[MAXLINE];
	char astr[MAXLINE] = "it is respose body.";
	char cnt_len[MAXLINE];
	char method[MAXLINE];
...
*/
	pid = Fork();
	if(pid == 0){
		sprintf(cnt_len,"%d", strlen(cgiargs));
		...
		if(strcmp("GET", method) == 0){		
			...
			rio_readrestb(rio, buf);
      			Setenv("CONTENT_LENGTH",cnt_len,1);
		}
		else{
      			strcpy(astr,"ERR, METHOD is not POST or GET\n");
      		return ;
    		}
...
void requestHandle(int connfd, double arrivalTime)
{
  ...
  printf("%s %s %s\n", method, uri, version);
  Setenv("REQUEST_METHOD",method,1);// insert method 
